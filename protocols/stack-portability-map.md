...
Examples (from agent-dashboard's empirical record):
...
|---|---|---|
| Node/TS + Drizzle (any driver) | `db.select() / .insert() / .update() / .delete() / .batch() / .transaction()` chain | record into structured state; mirror atomic primitives. See [agent-dashboard fake-db.ts](agent-dashboard/tests/unit/_helpers/fake-db.ts). |
...
|---|---|
| Node/TS + OAuth + session cookie | `getSession()` returns dummy session when `TEST_AUTH_BYPASS=1 && NODE_ENV !== 'production'`. (current agent-dashboard reference) |
