...
**Active-agent-surface clarifier.** "Agent file removed from `agents/`" above refers to a **live, top-level** agent contract (`agents/*.md`) — the dispatchable surface a consumer can rely on. Moving a not-yet-live stub between `agents/_planned/` and `agents/_archive/` (e.g. a `_planned → _archive` promotion or supersession move), or removing a superseded stub, is **not** the removal of a consumer-visible agent and is **not** MAJOR by that clause. These sub-namespaces are not registry-loaded and not part of the consumer-visible active-agent contract surface (and `_archive/**` is *additionally* excluded from the published tarball per `scripts/sync-src/manifest.json5`; `_planned/**` does ship in the tarball), so a change confined to them cannot break a downstream consumer. Retiring or demoting a **live** top-level agent into those sub-namespaces still crosses the boundary off the active surface and remains MAJOR. See §4.2 invariant 3 for the mechanical keying (renames keyed on the OLD path).
...
3. **Severity-floor heuristic (active-agent-surface keyed)** — the floor is computed over the **consumer-visible active surface** of the versioned directories. For `agents/`, that surface is the **top-level dispatchable contracts (`agents/*.md`)** — the files the plugin registry loads and that ship as live contracts; the sub-namespaces `agents/_planned/**` (not-yet-dispatchable stubs) and `agents/_archive/**` (internal history, excluded from the published tarball per `scripts/sync-src/manifest.json5`) are **NOT** part of it. `commands/`, `protocols/`, `templates/`, `hooks/`, and `scripts/` are active-surface in full. If the staged diff removes/renames a file **on the active surface**, the bump must be MAJOR — and because a rename is keyed on its **OLD path**, retiring a live agent (`agents/x.md → agents/_archive/…`) or demoting one (`agents/x.md → agents/_planned/x.md`) stays MAJOR, while a move confined to the non-active sub-namespaces (a `_planned → _archive` stub promotion or supersession) does not floor at MAJOR — such a move **on its own carries a PATCH floor** (it is non-breaking; the floor is a *floor*, so the release author may still classify higher, e.g. MINOR when the same release also adds a live agent). If the diff only adds active-surface files (a new top-level `agents/<name>.md`, or the additive side of a rename INTO `agents/`) with no active-surface removals or renames, MINOR is the floor. The hook does not enforce the ceiling (a release author can always choose to over-classify); it enforces the floor.
...
1. Diffs `package.json` against `origin/main` (the default `--base`), not against a tag
