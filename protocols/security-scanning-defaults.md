...
- **Pre-commit hook:** runs locally so secrets never land in commit history (much harder to remove from history than to prevent up-front).
- **CI gate:** also runs in CI as defense-in-depth — a developer who skipped the pre-commit hook gets caught here.
...
### A.1 Node.js / TypeScript (current agent-dashboard reference)
...
| Secret | **gitleaks** (mature, fast, ships pre-built rules) | Pre-commit hook + `.github/workflows/gitleaks.yml` |
