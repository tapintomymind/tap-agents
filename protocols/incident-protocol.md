...
Defines the bug-to-incident learning loop — how a runtime error captured in <project> (Tier 2) becomes institutional memory in the Tier 1 framework.
...
Per `<project>/src/lib/error-capture.ts §"SANITIZATION CONTRACT"` (path reflects post-2026-05-14 BL-059 cascade-rename; was `<project>/`). Treat this as a security boundary, not a hint.
