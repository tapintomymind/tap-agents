...
Adding a first `bin` is a net-new export surface with nothing removed, so this is a framework MINOR per `versioning-protocol.md §3.2`. Coverage is 26 cases on Node's built-in test runner against a mock server implementing the contract — happy path, every pending/error/rate-limit branch, the timeout ceiling, the file perms and exact shape and round-trip keys, and the subcommand behaviors — with zero new dependencies and a fully deterministic fake clock. This release bundles the public-mirror privacy sweep alongside the CLI; the sweep is doc-hygiene only and carries no team-shape change, so it has no separate narrative entry here (see the v0.28.0 `CHANGELOG.md` entry for its technical detail).

Two holds matter here. As with the recent telemetry releases this is staged, not yet published, pending the operator's distribution decision; but it is additionally gated on the companion app work, because the CLI's endpoints do not exist in production yet. They are a separate app-side unit (a device-codes table, three endpoints, and a browser approve page) sitting behind an ops-security review and the multi-tenant isolation gate that must close before any client-facing exposure. Publishing the CLI before those endpoints deploy would ship a login that calls nothing. So the release is fully prepped — code, tests, all three version channels, and this narrative — but explicitly not tagged or published until the endpoints are live and an end-to-end round-trip is green. Adoption, when it comes, flows through the dedicated `sync-tapagents` branch, not directly through dev.

Cross-reference: `CHANGELOG.md` v0.28.0 entry; `workspace/_global/tapagents-login-device-auth-contract-2026-06-02.md` (the frozen wire contract this implements).
