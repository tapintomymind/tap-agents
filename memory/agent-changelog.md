...
## 2026-05-12 — Session-tracking goes from manual to default (v0.15.0)

**Framework discipline shift.** `session-coordination-protocol.md` Rule 1 required every cross-cutting session to write a YAML block to `active-sessions.md` and seal it manually. The premise was honest discipline. The empirical result: weeks passed with zero new entries while multiple concurrent feature branches collided on the same version slot and parallel sessions independently dispatched architect to fix the same backlog item. Manual discipline failed not because operators were lazy but because the entrance fee was wrong — every cross-cutting session paid an opening cost (write the block) AND a closing cost (remember to seal), and the framework didn't make either cost zero.
...
## 2026-05-11 — docs/ in npm tarball + Trusted Publishing migration (v0.8.3)

**Two-axis infrastructure patch.** v0.8.3 bundles two independent PATCH-grade improvements that share the same release surface: adding `docs/` to the npm package's `files` allow-list, and swapping the publish workflow's auth mechanism from a long-lived `NPM_TOKEN` secret to OIDC-based Trusted Publishing. Both are publish-mechanic changes — no agent prompts, protocols, commands, templates, hooks, or settings change. The npm package's programmatic API surface widens only by the addition of `docs/*` paths.

**Closing the `scaffold-overlay/` gap.** When `agent-dashboard` migrated from cross-repo sync to npm dependency in the 2026-05-11 session, two framework-canonical docs (`docs/managed-agents-comparison.md`, `docs/specs/2026-05-04-claude-team-design.md`) ended up in `agent-dashboard/scaffold-overlay/docs/` because the npm package's `files` allow-list didn't include `docs/`. v0.8.3 closes that gap. Once `agent-dashboard` runs `npm install @tapintomymind/tap-agents@^0.8.3`, those two files can be deleted from its `scaffold-overlay/` directory — they now arrive via the canonical channel.

**Trusted Publishing — second leg of npm hardening.** The initial publish pipeline (v0.8.0 through v0.8.2) used a long-lived `NPM_TOKEN` automation token stored as a repo secret. Trusted Publishing replaces this with OIDC: the GitHub Actions workflow proves its identity to npm cryptographically via the existing `id-token: write` permission, and npm verifies that the workflow's `repository: tapintomymind/tap-agents` + `workflow: publish.yml` matches the configured trusted publisher. No secret to rotate, no exposure if a workflow log leaks, no manual revocation when the team changes. Setup is done on npmjs.com → package settings → Trusted Publishers BEFORE this release's tag push so the first OIDC publish has the configured trust anchor.

**Migration sequence.** v0.8.3 is the first release that exercises Trusted Publishing in production. The `NPM_TOKEN` secret remains in place during the cutover as a fallback — if OIDC publish fails for any reason (misconfiguration, npm-side outage), we'd see a clear auth error in the workflow logs, can patch the trusted-publisher config, and retry. After v0.8.3 publishes successfully via OIDC, the `NPM_TOKEN` secrets across `tap-agents` and `tap-agents-internal` repos can be deleted as a follow-up cleanup.

**Related cleanup landing in the same window.** Outside the release commit but in the same operator session: the `TAP_AGENTS_RO_TOKEN` GitHub PAT (previously used by `agent-dashboard/.github/workflows/sync-scaffold-source.yml` for cross-repo checkout) is being revoked. That workflow was deleted in the 2026-05-11 dashboard migration commit; the PAT is now orphaned. Revoking it eliminates a stale credential that no longer serves a workflow.

**Provenance.** User direction 2026-05-11 — "Revoke TAP_AGENTS_RO_TOKEN. Migrate to npm Trusted Publishing." Both items were tracked as pending follow-ups; consolidating them into v0.8.3 makes the cleanup atomic. Per `protocols/versioning-protocol.md` §3.1, PATCH is correct: no agents/commands/protocols/templates/hooks added or removed, no consumer-visible behavior change in the package's programmatic API. The `docs/*` addition expands the export surface without breaking existing imports (additive). The Trusted Publishing migration is pure CI infrastructure invisible to consumers.
