...
## 2026-05-29 — Framework v0.27.0 — Credential-file read in _telemetry.py: onboarding-enablement (M-D slice A0)

The onboarding-enablement prerequisite that sits just before the cloud-mirror slices lands as Slice A0. The cloud telemetry mirror introduced in v0.24.0 and wired across v0.25.0–v0.26.0 only activates when `TAPAGENTS_LIVE_TOKEN` is set in the environment — which meant a fresh deployment couldn't start syncing to the dashboard without an operator exporting an env var and relaunching the whole session tree. A0 removes that step: `emit_event_http()`'s token and ingest-URL resolution now reads fresh on every flush from a precedence chain — environment (`TAPAGENTS_LIVE_TOKEN` / `TAPAGENTS_LIVE_INGEST_URL`) → credential file `~/.config/tapagents/credentials.json` → built-in default URL — instead of from the environment alone.

The load-bearing reasoning is why a file read removes the restart. Environment variables are inherited at process launch, so changing the token previously required relaunching the whole Claude Code process tree. A file read inside the flush path has no such constraint: each hook fires as a fresh `python` subprocess that reads the file directly on its own threshold/atexit flush, so a token written mid-session — by hand, by an operator demo, or by a future `tapagents login` — is picked up by the very next hook fire, with no `export` and no restart. This extends the existing read-at-flush-time discipline (the env var was already read at flush, not at import) rather than changing that timing.

Backward-compatibility is total and the fail-open contract is preserved end-to-end. Environment wins: when `TAPAGENTS_LIVE_TOKEN` is set the resolver short-circuits and never stats the file, so behavior is byte-identical to v0.26.0, and existing operator setups and CI overrides are unchanged. The credential file is purely the new default for humans who never want to touch env. A missing file, an unreadable file, malformed JSON, wrong-shape JSON, or a non-string `token` / `ingest_url` field all degrade to the exact no-op of a missing env var today — warn-once to stderr, cloud mirror disabled, local `emit_event()` continuing as the source of truth — and `Path.home()` raising is swallowed too; the resolver never raises.

The change is additive and backwards-compatible (`versioning-protocol.md §3.2`): a new module-level `_resolve_credentials()` helper plus a new precedence tier inside the already-wired hook, with no file removed or renamed, no public API or export-shape change, no `settings.json` change, and no `live_events` schema change — which makes the release a framework MINOR. Billing stays Pool A: a pure stdlib file `stat`/`read` plus the existing app→app `emit_event_http()` POST, with no `claude` invocation, no Anthropic SDK, and no `api.anthropic.com` call. Coverage: `scripts/test-credential-resolution.py` (13 stdlib unittest cases — the full env→file→default precedence matrix, the fail-open contract, and an end-to-end flush that POSTs with the file's bearer token and ingest URL when no env var is set), plus a hardening pass on `scripts/test-emit-event-http.py` so its env-clearing tests isolate `XDG_CONFIG_HOME` and never pick up a real credential file on a dev machine. No devDeps added.

Cross-reference: `CHANGELOG.md` v0.27.0 entry; `workspace/_global/frictionless-telemetry-sync-onboarding-2026-05-29.md` (Slice A0 credential model + read-path spec).
