...
## 2026-05-19 — Framework v0.23.0 — Framework expansion: Context7 + four new protocols + two new Tier 1 agents + multi-host architecture

v0.23.0 is the framework's largest team-shape expansion since the dual-channel-distributable cut at v0.8.0. Two new active Tier 1 agents land, four new protocols compose into a single feedback-triage architecture, and a metadata-first subsystem opens the door to multi-host runtime targeting without breaking the Claude-only baseline.

**The new agents.** Marketing Designer activates as a sibling-to-Designer Tier 1 role that owns visual, IA, and conversion design for public marketing surfaces — hero composition, feature blocks, CTA hierarchy, scroll narrative, footer + meta, and competitor-as-conversion-machine evaluation. It is intentionally distinct from product-flavored Designer (whose surface is inside-the-app UI tokens, component patterns, IA for authenticated flows). Industry Researcher activates as a deeper-than-Strategist competitive analyst — market sizing, trend monitoring, regulatory landscape, per-competitor moat decomposition, and source-quality grading. It supplements rather than replaces Strategist's first-pass scan; Strategist's algorithm gains an Operating Principle 8 that defers deep work to industry-researcher when active. Both agents ship with three trigger lanes (briefed/scoping default for project-class match, operator-driven via direct slash command, Critic-signal portfolio recurrence) and dedicated slash commands. A new `/pmm` direct invocation ships alongside.

**The four new protocols compose as one feedback-triage rollout.** Decision-class taxonomy classifies every Open Question by who has authority to resolve: operator-blocking (`operational | strategic`) gate engineering dispatch; ESCALATED (`commercial | clinical | legal`) do NOT gate dispatch but route to non-operator resolvers with a named workaround. EA renders the two surfaces in separate Decision Packet sections with a cardinal-zero rule for empty ESCALATED sections. V2 roadmap anchoring asks Architect to classify each V-item: `architecture-now` (all three triggers must hold) gets a four-field entry in a new reserved `tech-strategy.md §"Architecture-now V-anchors"` section at roadmap-creation time; `architecture-deferred` (any one trigger fires) carries a one-line defer reason. PRD addendum pattern asks Strategist to classify each PRD revision as in-place rewrite vs. parallel addendum based on six trigger fingerprints (three on each side), with default-when-uncertain being revision. Workstream index codifies a per-project reading-order map at `workspace/<slug>/workstream-index.md` that Conductor maintains on phase transitions and Backlog Curator flags for drift in Phase B.2. Critic absorbs all four protocols as the new Phase B.1 four-axis bundle: `depth_assessment` (research-class artifacts), `decision_class` (OQ classification verification), `V-anchor` (V-item classification + anchor presence), `addendum_vs_revision` (PRD revision-pass classification verification). Eight existing active-tier agents pick up prompt-version bumps in lockstep to wire these protocols into their algorithms.

**The Context7 capability accelerant.** A new `protocols/docs-research-protocol.md` codifies a single-call docs lookup contract for inline library/framework reference queries via the Context7 MCP, with graceful degradation when the MCP server is unavailable. Citation protocol gains a `[context7]` row in §Tags so docs-research-sourced claims are auditably distinct from WebSearch / WebFetch citations. The protocol's telemetry event spec is reserved in `protocols/telemetry-events.md §2.5` as SPEC ONLY in this release — the hook implementation lands in a future release. Strategist and Architect both pick up read-list pointers to the protocol.

**Constrained Implementation Mode canonicalized.** `protocols/dispatch-efficiency.md §7` becomes the canonical reference for the file-boxed + proof-boxed micro-slice dispatch contract. The section spec includes the canonical reusable template (`Mode: constrained` + `Slice ID:` + `Outcome:` + `Allowed paths:` + `Denied paths:` + `First proof by minute N:` + `Heartbeat every N minutes:` + `Stop and report if:` + `Verification:` + `Reportback fields:`), the preflight echo requirement, the drift-detection contract, and the re-dispatch shape. Architect gains a full "Implementation-brief constrained-mode requirement" section that explicitly distinguishes when constrained mode is the right cut (narrow slices, high-drift surfaces, post-drift re-dispatch) vs. when default mode is the right cut (broad multi-surface milestones, greenfield exploration). The `_baseline/` Tier 2 templates ship the routing + preflight-echo enforcement contracts so future projects scaffolded against baseline inherit the constrained-mode discipline out of the box.

**Runtime-adapters metadata subsystem.** A new top-level `runtime-adapters/` directory becomes the canonical home for declarative-only per-host adapter contracts. Three shared schemas land: `adapter-contract.json` (JSON Schema for `adapter.json` files), `manifest-v2.schema.json` (future generated manifest shape), `telemetry-fields.json` (runtime-aware telemetry fields). Two host adapters land: Claude adapter ships `legacy-manifest` mode (status `stable-existing`); Codex adapter ships `generated-files` default mode + `plugin-beta` non-default mode (status `planning-placeholder`). Per-host templates accompany each adapter. Three new scripts back the subsystem: `render-runtime-adapter.ts` reads adapter metadata, validates against schema, and emits files per the selected mode (long-term path for all hosts); `test-runtime-adapters.ts` is the audit-discipline guard that refuses any provider SDK import, REST endpoint, credential-like field, or inline bearer token in adapter files; `sync-codex.ts` is the interim Claude → Codex regeneration path planned for absorption into `render-runtime-adapter.ts` post-architecture-flip. The discipline contract is strict — runtime-adapters/ files are declarative-only, and a violation surfaces as a test failure before any release.

**Permission-denial telemetry capture (observational).** A new `hooks/permission-denial-capture.py` hook chains PostToolUse + PostToolUseFailure + PermissionDenied events. It captures tool calls whose response indicates a permission denial (Claude Code permission mode, settings.json deny rules, framework hook blocks) and emits a `harness-or-permission` block event to `events.jsonl`. The hook is purely observational — exit 0 always; never blocks. It is the primary capture surface for framework-hook firings (the PreToolUse exit-2 paths from `orchestrator-dispatch-gate.py`, `pre-tool-gate.py`, `version-gate.py`). The `hooks/_telemetry.py` shared library gains an `emit_harness_block()` helper. A focused test script provides telemetry privacy + classification coverage.

**First Next.js stack template.** `templates/stacks/nextjs/react-component-agent.md` is the first stack-specific frontend worker template, alongside a `README.md` that declares the partial-stack baseline-fallback contract. Future Architects scaffolding Next.js Tier 2 sets now overlay this template's roles before falling through to baseline. Four new authoring-surface templates also land: `competitor-deep-dive.md`, `competitor-eval.md`, `marketing-design-spec.md`, `knowledge-base.md`. The PRD addendum template (`prd-addendum.md`) completes the prd-addendum-pattern protocol's template surface.

**New planned-stub: knowledge-curator.** Sibling-to-Backlog-Curator (curator-lite). Owns user-narrative-grade project context at `workspace/<slug>/knowledge-base.md`. Stub-only in this release — not activated. Three activation triggers documented for the future promotion.

**Why MINOR (not MAJOR).** Eight existing active-tier agents bump `prompt_version`, but every change is additive (new algorithm steps, new sections, new Operating Principles). No agent removed, no command removed, no protocol removed, no hook authenticity marker removed, no `fires_when` trigger narrowed. The two new active agents add new dispatch paths but no existing path is removed or narrowed. Adopters running the framework gain new behaviors on the next dispatch; nothing breaks at the prior version. Per `protocols/versioning-protocol.md §3.2`, the bump is MINOR.

**Known follow-up.** Thirteen protocols + three scripts ship with `tapagents-app (formerly agent-dashboard pre-2026-05-14 BL-059)` phrasing — load-bearing project-slug references with internal IDs and internal dates that ship verbatim to npm-tarball consumers. Per `feedback_framework_changelog_audience_discipline` (2026-05-17), public artifacts shipping in the tarball should avoid project slugs and internal user-decision dates. A brand-integrity sweep is planned within ~48h as a patch release.

Cross-reference: CHANGELOG.md v0.23.0 entry; PR [#5](https://github.com/tapintomymind/tap-agents/pull/5).
...
**Why this matters at the team-shape layer.** Before today, the framework had one consumer — the framework operator running it locally — and version drift between `tap-agents` and `agent-dashboard/scaffold-source/` was managed by hand-copying. With two real external channels live, the framework's `version` field crosses a threshold from internal-discipline into external-contract. Downstream consumers (`agent-dashboard`'s Vercel build, plugin-marketplace users) rely on the number being honest. If a "patch" silently renames an agent, downstream breaks without warning.
...
**Provenance:** Conversation chapter "CLI/marketplace implementation" (2026-05-11). Strategic framing arc: agent-dashboard → tapagents marketplace question → RuFlo competitive analysis (`ruvnet/ruflo` reviewed) → decision to take the focused-team positioning + Claude Code marketplace channel over RuFlo's 32-plugin breadth → scaffold-source deduplication requirement → npm package as canonical egress → strict SemVer discipline as the contract that lets the dual channels work.
...
**Provenance:** Triggered by agent-dashboard's Tier 2 metrics surface design — the dashboard's per-project framework activity panel needed a structured event stream to render. The protocol was the right shape to extract upward into the framework rather than implementing the contract twice. First end-to-end exercise of the protocol shipped same day in agent-dashboard `dev`.

---

## 2026-05-07 — End-of-session: BL-034 M1.5 dispatch + Curator reconcile + ip-protection intake park
...
**EA-style briefing delivered.** Full cross-project status briefed to user: 5 decisions queued, 34-item backlog summary with counts (P0=0 P1=15 P2=14 P3=5; open=19 in-progress=4 done=11), active sessions inventory, project health for claude-team-app (handed-off, Tier 2 in flight) and ip-protection (briefed, awaiting user approval).

**ip-protection intake parked.** User elected to defer the ip-protection brief review ("we'll discuss later"). No state change; `ip-protection/state.json` remains `briefed` with `blocked_on: user approval of intake-brief.md`. Re-surfaces via normal staleness cadence on next daily sweep.
...
**Mirror parity:** all 10 framework files (1 protocol + 5 agents + 4 templates) mirrored to `agent-dashboard/scaffold-source/`; `diff -q` empty post-write across all pairs.
...
**Industry portability honored.** Scaffold-source mirrors at `agent-dashboard/scaffold-source/agents/{conductor,executive-assistant,org-designer,backlog-curator}.md` updated identically. `diff -q` empty post-edit on all 4. Stub also removed from `agent-dashboard/scaffold-source/agents/_planned/`. The curator-lite scope is stack-agnostic — greenfield projects spinning up via scaffold inherit Curator activated.
...
User surfaced session-tracking drift at 2026-05-06T21:00: BL-023 (16-00) and BL-013 Phase 5 (18-15) sessions both shipped to prod via `agent-dashboard` merge `bf2f08a` at 19:49 but their `active-sessions.md` entries were never sealed — left `status: in-progress` for ~3.5h. User: *"The ones in flight, confirm they finished. I thought I finished both those. Add in the necessary processes to track it better as needed."*
...
**Trigger.** Two approved, execution-ready execution plans (`agent-dashboard/.claude/docs/v1.5-execution-plan.md` Task 7 and `workspace/agent-dashboard/decision-packet-bl013-multiuser-security.md` Phase 3.5) both touched the same surface — `POST /api/webhooks/github`. The v1.5 plan's inline note ("deduplicate vs BL-013 step 5, do not duplicate plumbing") was correct in spirit but ambiguous in arbitration. Without an explicit ownership statement, both Ops/Security (dispatched into BL-013 Phase 3.5) and the Tier 2 implementer (dispatched into v1.5 Task 7) could reasonably believe they own implementation. This entry summarizes the resolution.
...
- Atomic-cadence source: `~/.claude/projects/-Users-tapandesai-Desktop-tapintomymind-App-Development/memory/feedback_changelog_proactive.md`.
...
**Cross-references.** v1.5 execution plan at `agent-dashboard/.claude/docs/v1.5-execution-plan.md` Phase 0.1; promotion proposal at `agents/_planned/_proposals/qe-promotion-2026-05-06.md`.

---

## 2026-05-06 — Tier B audit: agent-dashboard `dev → main` promotion (M1 → M2 cutover, 21-commit merge)
...
**Trigger:** User flagged token waste during the agent-dashboard session (~12 subagent dispatches with redundant 64KB context inlined and model-mismatched defaults). Org Designer dispatched for audit + codification.
...
**Trigger:** First Tier 2 project (TapHQ / agent-dashboard) reached production-ready state. User completed Vercel Pro signup, Neon DB setup, GitHub App registration. Production URL went live: https://agents-dashboard-olive.vercel.app.

**What changed:**
1. **Production deployment infrastructure** — Vercel Pro project linked to `tapintomymind/agent-dashboard`, env vars configured (10 vars including encrypted token key + GitHub App private key inline-PEM), GitHub App callback URLs registered for prod URL, `maxDuration=120` set on the create-project route to clear Vercel Pro's 60s default ceiling. Custom domain `hq.tapintomymind.com` queued (DNS + SSL pending).
2. **First production incident captured + resolved** — `scaffold_build_failed (500)` on initial smoke test. Root cause: `build-tree.ts` hardcoded a Mac filesystem path that doesn't exist on Vercel serverless. Fix: bundle Tier 1 framework into `agent-dashboard/scaffold-source/` at build time, committed snapshot to repo, prebuild script made tolerant of missing source. See `memory/incidents.md` §1 for the full incident.
3. **Cross-tier bug observability system** — new mechanism for capturing failures and routing them through the team's learning loop:
   - **Tier 2 (agent-dashboard):** new `bug_reports` table, `withErrorCapture` middleware wrapping API routes, `/admin/bugs` admin view (gated to `tapintomymind`), `error.tsx` global client error boundary, `/api/bugs/report` ingest, `/api/bugs/[id]/promote` returns formatted markdown for manual paste into incidents.md.
...
**What changed (Tier 2 / agent-dashboard):**
1. **NEW:** `src/app/api/admin/test-error/route.ts` — observability ping endpoint. Admin-gated (404 not 403, same pattern as `/admin/bugs`). `?type=throw` triggers a thrown Error; `?type=500` returns a 5xx response. Both paths get captured by `withErrorCapture` → written to `bug_reports` table. Standard pattern — every observability stack has one (Sentry's "Send a test event," Datadog synthetic checks). Commit: `fe22b4c` on agent-dashboard main.
2. **NEW:** `dev` branch on `tapintomymind/agent-dashboard`. Workflow now: local development on `dev`, push to `dev` triggers Vercel preview deployment, `dev` → `main` merge triggers Vercel production deployment. Local → QA → Prod rhythm formalized.
...
User branch strategy clarified mid-session: `main` = prod, `dev` = local + qa, `feature` = isolated. Tap-agents framework currently on `main` only; agent-dashboard uses dev → main flow.
...
2. **Cross-tier infrastructure shipped (agent-dashboard, `dev` branch):**
...
2. **NEW:** `workspace/_global/backlog.json` — machine-readable mirror. 12 items seeded (3 Tier 1, 9 Tier 2 / agent-dashboard). EA reads counts from this file; Conductor reads items for dispatch pulls. Schema documented inline.
...
4. **NEW:** `workspace/agent-dashboard/backlog.md` — Tier 2 example backlog seeded with 8 agent-dashboard-specific items from tonight: M2 real-data hookup (in-progress), custom domain, wildcard GitHub App callback, M3 Decision Packet UI, additional pages real-data hookup, vercel-setup.md NEON_BRANCH doc update, Playwright 90s ceiling test, M4 landing page + demo video.
...
**Action:** `npx vercel env run -e production -- npx drizzle-kit push` against the linked Vercel project (`tapintomyminds-projects/agents-dashboard`). Used `vercel env run` (in-memory env injection) instead of `vercel env pull` (file-on-disk) per the file-deletion-safety memory and to avoid prod creds sitting on disk even briefly.
...
1. **Updated `.env.local`** — pulled `local` branch connection string via `neonctl connection-string local --pooled`, substituted into `.env.local` via shell-pipe → python regex (no creds in conversation). Backup saved at `agent-dashboard/.env.local.bak.1778042003`. Set `NEON_BRANCH=local`.
...
- `agent-dashboard/scripts/promote-to-prod.sh` (Gate 2.5 inserted; two-step operator flow for db-admin sentinel-verify)
