...
**Status:** Draft 2026-05-18. Promote to `protocols/` per §6 once the typology earns adoption.
...
TapAgents lives inside a growing ecosystem of external repos, plugins, MCP servers, and curated reference sets that *could* enhance the framework. Without a consistent evaluation rubric, each "should we use this?" call becomes ad-hoc — could lead to vendor lock-in, missed token-savings, or contract-discipline violations. The typology is preventive — no such incident has occurred to date; this document codifies discipline before the failure mode surfaces, not in response to it.
...
These thresholds are calibration heuristics from the 2026-05-18 evaluation pass; tighten or loosen per §5 as additional passes accumulate.
...
Worked examples below cite specific vendor repos as current-evaluation examples. The shape definitions in §2 are vendor-agnostic; future evaluations may produce different vendor-specific examples per category. Treat the named repos as illustrative for the 2026-05-18 pass, not as the canonical or only fit for each shape.

### §4.1 upstash/context7 — capability accelerant

- **Shape:** Capability accelerant (version-pinned library docs via MCP).
- **Earns the cost?** Yes — replaces WebSearch + multiple WebFetches with one `query-docs` call. Typical library-API research touches WebSearch + 2-3 WebFetch calls (per `protocols/docs-research-protocol.md §5`) — meets the ≥3-call threshold. Order-of-magnitude estimate in `protocols/docs-research-protocol.md §5` (pending production measurement per the `docs-research-call` telemetry event).
...
This document lives in `docs/` while the typology is on its first deployment. Promote to `protocols/external-ecosystem-typology.md` the first time this routing test is applied to a candidate by an agent or contributor other than the 2026-05-18 evaluator. Single use by anyone-not-the-author is the bar; if it survives contact with a different reader, it has earned protocol status.
