...
**What changed in the versioning contract.** The severity floor — enforced at Gate 2 (`hooks/version-gate.py`, local commit-time) and Gate 3 (`scripts/build-src/version-check.ts`, the `version-check.yml` CI release gate), and specified in `protocols/versioning-protocol.md §4.2` invariant 3 + the §4 active-agent-surface clarifier — is now computed over the **consumer-visible active surface** of the versioned directories rather than the raw file tree. For `agents/`, the active surface is the top-level dispatchable contracts (`agents/*.md`); the sub-namespaces `agents/_planned/**` (not-yet-dispatchable stubs) and `agents/_archive/**` (internal history) are NOT part of it. `commands/`, `protocols/`, `templates/`, `hooks/`, `scripts/` remain active-surface in full. Renames are keyed on the **OLD path**: retiring or demoting a **live** top-level agent (`agents/x.md → agents/_archive/…`, or `→ agents/_planned/x.md`) still crosses off the active surface and remains **MAJOR**; a move confined to the non-active sub-namespaces (a `_planned → _archive` stub promotion or supersession) is now **additive-class** and does not floor at MAJOR. Before this change, the gate treated any `D`/`R` anywhere under `agents/` as MAJOR — which both forced a spurious `0.x → 1.0.0` bump on a legitimate stub promotion AND (the defect this closes) let a live-agent removal RENAMED to a path outside the versioned dirs escape MAJOR, because the floor keyed the rename on its new path. Keying removals on the OLD path fixes both directions at once.

**Retro-move: the Shape A′ deviation is retired.** v0.37.0 activated `release-coordinator` but — because the pre-carve-out floor would have forced a spurious major on the archive-move — retained the promoted stub IN PLACE at `agents/_planned/release-coordinator.md` with a PROMOTED marker (the one-time "Shape A′" exception). With the floor re-keyed, that deviation is unwound: the stub is moved to `agents/_archive/release-coordinator-promoted-2026-07-01.md` under the standard `rename-on-move` doctrine. Future promotions use `rename-on-move` directly — no in-place-marker exception. **The move changes the published tarball by exactly one file:** `agents/_planned/release-coordinator.md` — which shipped in v0.37.0/v0.37.1 as the PROMOTED tombstone — **leaves the tarball**, and its archived replacement does NOT re-enter it (`scripts/sync-src/manifest.json5` excludes `agents/_archive/**/*` from the published package; only `agents/_archive/README.md` is kept). This is **non-breaking**: the departing file was a not-yet-dispatchable stub tombstone, never registry-loaded, and the live dispatchable contract `agents/release-coordinator.md` is unchanged. The removal is invisible to the plugin registry and to any adopter that dispatches agents — only a byte-level tarball diff surfaces the tombstone's departure.

### Changed

- **`hooks/version-gate.py`** — `_classify_severity_floor` re-keyed to the consumer-visible active surface via the new `_is_active_surface` helper + `_AGENT_SUBNAMESPACE_PREFIXES` (`agents/_planned/`, `agents/_archive/`). `_staged_diff_files` now returns `(status, old_path, new_path)` — retaining BOTH paths so removals/renames key on the OLD path and additions on the NEW path. Net behavior: retiring/demoting a live agent stays MAJOR; a `_planned → _archive` move is additive-class.
- **`scripts/build-src/version-check.ts`** — the CI counterpart (Gate 3) receives the identical re-key: `isActiveSurface` + `AGENT_SUBNAMESPACE_PREFIXES`, and `getDiffEntries` retains `oldPath`/`newPath`. Local hook and CI gate now agree on floor semantics.
- **`protocols/versioning-protocol.md`** — §4.2 invariant 3 severity-floor heuristic re-keyed to the active-agent surface (renames keyed on the OLD path); §4 gains the active-agent-surface clarifier distinguishing a live top-level agent removal (MAJOR) from a non-active sub-namespace move (additive-class).
- **`agents/_planned/release-coordinator.md` → `agents/_archive/release-coordinator-promoted-2026-07-01.md`** — rename-on-move: the promoted stub is archived under the conventional `<role>-promoted-YYYY-MM-DD.md` name, retiring the Shape A′ retain-in-place deviation. Non-active sub-namespace move — additive-class under the new floor; tarball-excluded per the manifest exclude `agents/_archive/**/*`.
- **`agents/_planned/README.md`** — the Shape A′ exception is marked resolved (2026-07-01); the `release-coordinator` row now points at `_archive/release-coordinator-promoted-2026-07-01.md`, and the doctrine note records that future promotions use rename-on-move directly.
- **`agents/_archive/README.md`** — documents that rename-on-move is now mechanically supported: a `_planned → _archive` move is additive-class and no longer burns a spurious major.
...
**Billing: Pool A.** Gate logic + protocol/README documentation + a file move only; no `claude` invocation, no Anthropic SDK, no `api.anthropic.com`.
...
**Activation criteria satisfied.** The stub's activation contract is met: the mechanical release floor the role operates within (tag-ancestry guard, Layer-B release flow, version-gate invariants, five-channel parity audit) was dogfooded across 18 releases since v0.24.0 without operator-side ambiguity about who owns the coordination decisions, and a production-readiness review of the full contract passed with zero blocking findings.

**Additive promotion.** The original planned stub is retained in place at `agents/_planned/release-coordinator.md` with a PROMOTED marker and a `promoted_to` pointer to the live contract — the consumer-visible surface gains an agent; nothing is removed. **Additive — adopters at v0.36.0 continue unchanged.** **[Superseded in v0.38.0:** the retain-in-place "Shape A′" posture described in this entry was unwound once the SemVer severity floor was re-keyed to the active-agent surface — the stub was retro-moved to `agents/_archive/release-coordinator-promoted-2026-07-01.md` via rename-on-move. This entry records what was true at v0.37.0.**]**

### Added

- **`agents/release-coordinator.md`** — the live contract (`status: active`, `prompt_version: 2026-07-01-1`). Owns the release lifecycle from `/release` invocation through Gate 5, stopping at successful npm publish + GitHub Release green: the Steps 1–9f execution arc (release-branch → PR → tag-on-merged-main → five-channel Gate 5, including the scripted manifest alignment and the hold path's second terminal state), Gate-5 failure diagnosis + incident reporting, trunk-state attestation, parallel-session release-order coordination (the release-lock coordination hook is documented as reserved / not-yet-implemented, pending a real collision), override-token justification routing, and hold-time `KNOWN_ORPHANS` annotation gated on the carry-forward integrity check. Everything past npm-publish-success and everything inside a consumer/adopter repo is explicitly out of its lane.

### Changed

- **`agents/_planned/release-coordinator.md`** — marked PROMOTED in place (`status: promoted`, `promoted_on: 2026-07-01`, `promoted_to: agents/release-coordinator.md`); deliberately NOT moved to `_archive/` — the in-place retention keeps the activation release additive in `agents/` (no removal or rename under a versioned directory) while the PROMOTED marker + pointer preserve the archive doctrine's goal: a reader immediately sees the file is a historical artifact and can find the live contract. The original stub body is preserved beneath the marker as the activation-decision audit trail. **[Superseded in v0.38.0:** the "deliberately NOT moved to `_archive/`" decision here was reversed by the active-agent-surface carve-out; the stub is now at `agents/_archive/release-coordinator-promoted-2026-07-01.md` (rename-on-move).**]**
- **`agents/_planned/README.md`** — stub count 11 → 10; `release-coordinator` moves from the Current-Stubs table to the Activated-agents table with a retained-in-place row, and the README documents the additive-promotion exception (a stub promoted this way stays at its `_planned/` path with the PROMOTED marker rather than moving to `_archive/`). **[Superseded in v0.38.0:** the additive-promotion exception was retired; the stub now lives at `_archive/release-coordinator-promoted-2026-07-01.md` and future promotions use rename-on-move directly. The README was updated to mark the Shape A′ exception resolved.**]**
- **`agents/executive-assistant.md`** — the `KNOWN_ORPHANS` exception now routes through the release-coordinator: EA surfaces the parity signal and routes the decision to the release-coordinator; EA never mutates the map itself. `prompt_version` → `2026-07-01-1`.
- **`commands/release.md`** — the hold path's `KNOWN_ORPHANS` annotation is now owned by the release-coordinator (the transfer clause that previously deferred ownership "if/when that stub is activated" has fired); each append remains user-approved per entry, and Org Designer retains the map-governance policy, which the release-coordinator executes but does not amend.
- **`protocols/versioning-protocol.md`** — §4.6's deliberate-hold posture now names the release-coordinator as the hold-time annotation owner.

### Fixed

- **`CHANGELOG.md`** — restored the `[0.29.0]` and `[0.29.1]` historical sections, which were lost in a 2026-06-02 full-sync regeneration; recovered byte-identical from git history.
...
**Downstream impact: none.** The only change is `CHANGELOG.md` (the `[0.31.0]` prose corrected to 6-path + this `[0.31.1]` entry added). No agent, command, protocol, template, hook, or config behavior changes. Adopters at v0.31.0 already have the correct 6-path `vercel.deploy-neutral.json`; this release changes nothing they execute.

**Billing: Pool A.** Documentation only; no `claude` invocation, no Anthropic SDK, no `api.anthropic.com`.

### Fixed

- **`CHANGELOG.md` — `[0.31.0]` "The mechanism is scaffold-shipped" paragraph** — the `ignoreCommand` example and its exclude-set description now match the shipped `vercel.deploy-neutral.json` (6 exclude paths: `.claude`, `hooks`, `scaffold-source`, `*.md`, `*.scaffold-meta.json`, `.bot-manifest.json`) rather than the superseded 4-path form. The config file itself was already correct in v0.31.0; only this narrative is corrected.

### Provenance

- Fix authored 2026-06-02 immediately after the v0.31.0 publish, when post-publish verification surfaced that a concurrent HQ spec refinement (the §3-complete 6-path `ignoreCommand`) had landed in the config + protocol + CHANGELOG prose at HQ but had been captured in the v0.31.0 release commit's CHANGELOG only partially (config + protocol updated; prose still 4-path). Per `protocols/versioning-protocol.md` Gate-5 / npm-immutability doctrine, the correction ships as the next version rather than a re-publish of v0.31.0.

## [0.31.0] — 2026-06-02 — Deploy neutrality: scaffold-shipped `vercel.json` `ignoreCommand` so framework-sync commits never redeploy a consumer environment

**Minor release. Additive — a new scaffold template file + new protocol/template/checklist content; no existing capability removed.** Makes every framework-sync commit (per the `sync-tapagents-protocol.md` §3 fingerprint) **deploy-neutral on every consumer environment, permanently** (user direction 2026-06-02). The `sync-tapagents → main` promotion and the `main → dev` back-merge each push a deploy branch, firing a consumer Vercel build. On `main` (prod) that rebuild redeploys byte-identical application output — merely wasteful. On `dev`/QA it is **dangerous**: the back-merge push rebuilds `dev`, which routinely carries ongoing, *unpromoted* application work, so a sync-triggered `dev` rebuild can redeploy in-flight work never approved for QA. This release removes both the waste and the hazard by scaffold-shipping a content-based build-skip keyed off the same §3 framework-scaffold paths.

**The mechanism is scaffold-shipped, not configured per project.** A committed `vercel.json` `ignoreCommand` — `git diff --quiet HEAD^ HEAD -- ':(exclude).claude' ':(exclude)hooks' ':(exclude)scaffold-source' ':(exclude)*.md' ':(exclude)*.scaffold-meta.json' ':(exclude).bot-manifest.json'` — asks "did anything outside the framework-scaffold paths change?" The exclude set is the full §3 framework-sync fingerprint minus the `package.json` carve-out: `.claude/`, top-level `hooks/` (Sig C hook payloads, tracked OUTSIDE `.claude/` on the real consumer), `scaffold-source/` (Sig B, for consumers that track it), `*.md`, the globbed `*.scaffold-meta.json` (matches the nested `scaffold-source/.scaffold-meta.json`), and `.bot-manifest.json`. Exit 0 (nothing outside changed) → Vercel SKIPS the deploy; non-zero → Vercel BUILDS. It is branch-agnostic (the same file protects prod, QA, and every preview because it lives on the branch and runs on every deploy of that branch) and fail-safe (an absent `HEAD^` errors non-zero → builds rather than wrongly skips). Because it ships from the Tier 2 deployment template, every NEW project scaffolded via TapAgents inherits deploy neutrality automatically; existing consumers adopt it the next time their deployment agent runs or via a merge-not-overwrite surgical commit (add ONLY the `ignoreCommand` key — never clobber a pre-existing `vercel.json` carrying `crons`/`functions`/etc.).

**The dependency-bump boundary is load-bearing.** `package.json` is deliberately NOT in the skip set — a framework *dependency* bump (Signature A) changes `package.json`, so it still builds and verifies, but on the isolated `sync-tapagents` **preview** only (per §4 Step 2), never via a silent auto-redeploy of `main`/`dev`. Deploy neutrality skips the *scaffold-copy* rebuilds (Steps 3 and 4 when the promoted content is pure scaffold); it does not skip dependency verification.

**Downstream impact: none (additive).** The new `templates/stacks/_baseline/vercel.deploy-neutral.json` is a copy-source consumed by the deployment agent at scaffold time; it does not change any existing consumer until that consumer's deployment agent installs it (merging into any pre-existing `vercel.json` rather than overwriting). The protocol/template/checklist additions are new content alongside existing material — no `fires_when`, authority, or output contract changed; nothing removed. Adopters at v0.30.2 continue to function unchanged.

**Billing: Pool A.** Documentation + a static JSON config template; no `claude` invocation, no Anthropic SDK, no `api.anthropic.com`.

### Added

- **`templates/stacks/_baseline/vercel.deploy-neutral.json`** — the canonical scaffold copy-source for the framework-sync deploy-skip `ignoreCommand`, as a concrete artifact so the mechanism cannot drift from the protocol prose. Carries the `$schema` + the simple-form `ignoreCommand` keyed off the four §3 scaffold-path globs (`.claude`, `*.md`, `.scaffold-meta.json`, `.bot-manifest.json`).

### Changed

- **`protocols/sync-tapagents-protocol.md`** — added §4.5 "Deploy neutrality (effective 2026-06-02)" (§4.5.1 the principle, §4.5.2 the dependency-bump boundary / host-agnostic contract, §4.5.3 the canonical Vercel `ignoreCommand` + verified behavior table + hardened `VERCEL_GIT_PREVIOUS_SHA` drop-in form, §4.5.4 host-agnostic equivalents for GitHub Actions `paths-ignore` / `netlify.toml ignore`, §4.5.5 scaffold-shipped provenance, §4.5.6 relationship to the rest of the protocol); added Layer F (deploy-time, scaffold-shipped) to §5 and §5.6; extended the §9 related-protocols table with the deployment-template + `vercel.deploy-neutral.json` rows. Purely additive — the five author-time/commit-time enforcement layers (A–E) and the §4 canonical flow are unchanged.
- **`templates/stacks/_baseline/tier2-deployment.md`** — added the "Deploy neutrality (framework-sync skip) — install at scaffold time" section instructing the deployment agent to install the `ignoreCommand` at scaffold time, **merging** into any pre-existing `vercel.json` (e.g., one carrying `crons`) rather than overwriting, with the dependency-bump-boundary caveat, shallow-clone hardening note, and host-agnostic equivalents.
- **`protocols/handoff-protocol.md`** — added a mechanical-checklist verification item confirming the deploy-neutral config landed at scaffold time (exit 0 on a scaffold-only diff, non-zero on an app/`package.json` diff), so scaffolding cannot silently omit the guard.

### Provenance

- Authored 2026-06-02 per user direction — *"framework syncs should be DEPLOY-NEUTRAL on every consumer environment, permanently."* The deploy-time complement to the branch-isolation discipline already in `sync-tapagents-protocol.md` §5 (Layers A–E): §5 keeps framework-sync content on the right branch; §4.5/Layer F makes that content invisible to the deploy pipeline once it lands, on every consumer environment uniformly. Boundary explicitly confirmed: pure scaffold syncs skip deploys everywhere; a framework *dependency* bump still builds on the isolated `sync-tapagents` preview only.
