...
**The mechanism is scaffold-shipped, not configured per project.** A committed `vercel.json` `ignoreCommand` — `git diff --quiet HEAD^ HEAD -- ':(exclude).claude' ':(exclude)*.md' ':(exclude).scaffold-meta.json' ':(exclude).bot-manifest.json'` — asks "did anything outside the framework-scaffold paths change?" Exit 0 (nothing outside changed) → Vercel SKIPS the deploy; non-zero → Vercel BUILDS. It is branch-agnostic (the same file protects prod, QA, and every preview because it lives on the branch and runs on every deploy of that branch) and fail-safe (an absent `HEAD^` errors non-zero → builds rather than wrongly skips). Because it ships from the Tier 2 deployment template, every NEW project scaffolded via TapAgents inherits deploy neutrality automatically; existing consumers adopt it the next time their deployment agent runs or via a one-line surgical commit.
